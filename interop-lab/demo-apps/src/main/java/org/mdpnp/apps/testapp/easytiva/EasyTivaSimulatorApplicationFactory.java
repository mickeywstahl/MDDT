...
public class EasyTivaSimulatorApplicationFactory implements IceApplicationProvider {
	
	private IceApplicationProvider.AppType type=new IceApplicationProvider.AppType(
			"EasyTiva Simulation", "NoET", (URL) EasyTivaSimulatorApplicationFactory.class.getResource("easytiva.png"), 0.75, false
		);
	...
	public IceApp create(ApplicationContext parentContext) throws IOException {
		...
		FXMLLoader loader = new FXMLLoader(EasyTivaSimulatorApplication.class.getResource("EasyTivaSimulatorApplication.fxml"));

        final Parent ui = loader.load();
       
        final EasyTivaSimulatorApplication controller = ((EasyTivaSimulatorApplication) loader.getController());

...
public class BisSimulatorApplicationFactory implements IceApplicationProvider {
	
	private IceApplicationProvider.AppType type=new IceApplicationProvider.AppType(
			"BIS Simulation", "NoBIS", (URL) BisSimulatorApplicationFactory.class.getResource("bis.jpg"), 0.75, false
		);
	...
	public IceApp create(ApplicationContext parentContext) throws IOException {
		...
		FXMLLoader loader = new FXMLLoader(BisSimulatorApplication.class.getResource("BisSimulatorApplication.fxml"));

        final Parent ui = loader.load();
       
        final BisSimulatorApplication controller = ((BisSimulatorApplication) loader.getController());

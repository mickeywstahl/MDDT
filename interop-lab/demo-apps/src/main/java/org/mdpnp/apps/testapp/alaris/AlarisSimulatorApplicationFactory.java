...
public class AlarisSimulatorApplicationFactory implements IceApplicationProvider {
	
	private IceApplicationProvider.AppType type=new IceApplicationProvider.AppType(
			"Alaris Simulation", "NoAlaris", (URL) AlarisSimulatorApplicationFactory.class.getResource("alaris_asena_pump.png"), 0.75, false
		);
	...
	public IceApp create(ApplicationContext parentContext) throws IOException {
		...
		FXMLLoader loader = new FXMLLoader(AlarisSimulatorApplicationFactory.class.getResource("AlarisSimulatorApplication.fxml"));

        final Parent ui = loader.load();
       
        final AlarisSimulatorApplication controller = ((AlarisSimulatorApplication) loader.getController());
